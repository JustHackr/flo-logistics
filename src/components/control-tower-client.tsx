...
import { AlertTriangle, ArrowUpRight, Boxes, CloudRain, Clock3, RefreshCw, Route, ScanBarcode, ShieldAlert, TrafficCone, Truck } from "lucide-react";
...
export function ControlTowerClient({ initialData }: { initialData: ControlTowerOverview }) {
  ...
  async function monitorSlaRisk(orderId: string) {
    const response = await fetch(withBasePath(`/api/risk/sla/${orderId}/monitor`), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ note: "Ops chose to monitor the prediction from Control Tower." }) });
    if (!response.ok) { setError("Unable to place the SLA prediction under monitoring."); return; }
    await refresh();
  }
