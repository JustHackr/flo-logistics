...
import { QR_FIXTURES, type QrFixture } from "@/lib/computer-vision/qr-verification";
import { listVideoInputDevices, type VideoInputDevice } from "@/lib/computer-vision/camera-devices";
import { withBasePath } from "@/lib/base-path";
import { useI18n } from "@/components/i18n/use-i18n";

type DetectorResult = { rawValue?: string; format?: string; boundingBox?: DOMRectReadOnly };
...
export function ParcelVerificationClient() {
  ...
  const loadHistory = React.useCallback(async () => {
    try {
      const response = await fetch(withBasePath("/api/computer-vision/barcode/history"), { cache: "no-store" });
      if (!response.ok) return;
      const body = await response.json() as { scans?: typeof history };
      setHistory(body.scans ?? []);
    } catch { /* History is useful context, not a blocker for scanning. */ }
  }, []);
  ...
  async function verify(code: string, mode: "fixture" | "live", fixture?: BarcodeFixture | QrFixture, format: "BARCODE" | "QR_CODE" = "BARCODE") {
    const normalized = code.trim().toUpperCase();
    if (!normalized) return;
    setLoading(true); setExceptionCreated(false); setCameraError(null);
    try {
      const query = new URLSearchParams({ code: normalized, mode, format });
  ...
  async function createException() {
    ...
    const response = await fetch(withBasePath("/api/computer-vision/barcode/exceptions"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: result.code, format: result.format, outcome: result.outcome, scanId: (result as BarcodeVerification & { scanId?: string }).scanId, reason: result.message }) });
    if (response.ok) { setExceptionCreated(true); void loadHistory(); }
  ...
  return (
    <div lang={locale} className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><div className="flex items-center gap-2"><FileImage className="h-4 w-4 text-primary" /><CardTitle className="text-base">{t("cv.parcel.fixtureGuide")}</CardTitle><Badge variant="outline">{t("cv.parcel.synthetic")}</Badge></div><CardDescription>{t("cv.parcel.fixtureGuideDescription")}</CardDescription></CardHeader><CardContent><div className="overflow-hidden rounded-xl border bg-muted/20"><img src={withBasePath("/cv/parcel-verification-fixtures.png")} alt="Generated parcel verification visual guide" className="h-auto w-full object-cover" /></div></CardContent></Card><Card><CardHeader><div className="flex items-center gap-2"><FileImage className="h-4 w-4 text-primary" /><CardTitle className="text-base">QR parcel photo samples</CardTitle><Badge variant="outline">SYNTHETIC</Badge></div><CardDescription>Use the exact QR cards below for scanning; these photos are visual context for a warehouse test.</CardDescription></CardHeader><CardContent><div className="overflow-hidden rounded-xl border bg-muted/20"><img src={withBasePath("/cv/parcel-qr-sample-guide.png")} alt="Generated parcel photo samples with blank label areas" className="h-auto w-full object-cover" /></div></CardContent></Card></div>

      <div className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]"><div className="space-y-4"><Card><CardHeader><CardTitle className="text-base">Generated barcode fixtures</CardTitle><CardDescription>{BARCODE_FIXTURES.length} test labels cover verified, review, unknown, duplicate, and damaged-package paths.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{BARCODE_FIXTURES.map((fixture) => <div key={fixture.id} className={`rounded-xl border p-3 ${fixture.id === selectedFixture.id ? "border-primary ring-1 ring-primary/30" : ""}`}><div className="mb-2 flex items-start justify-between gap-2"><div><p className="font-medium">{fixture.title}</p><p className="text-xs text-muted-foreground">{fixture.description}</p></div><Badge variant={fixture.kind === "verified" ? "outline" : fixture.kind === "unknown" ? "destructive" : "secondary"}>{fixture.kind.replaceAll("_", " ")}</Badge></div><div className="rounded-md border bg-white p-2"><img src={withBasePath(fixture.assetPath)} alt={`Barcode ${fixture.code}`} className="h-auto w-full" /></div><div className="mt-2 flex items-center justify-between gap-2 text-xs text-muted-foreground"><span>Lane: {fixture.expectedLane}</span><span>{fixture.packageCondition}</span></div><Button size="sm" className="mt-3 w-full" variant={fixture.id === selectedFixture.id ? "default" : "outline"} onClick={() => { selectFixture(fixture); void verify(fixture.code, "fixture", fixture); }}><Barcode className="mr-2 h-4 w-4" />Test this label</Button></div>)}</CardContent></Card><Card><CardHeader><div className="flex items-center gap-2"><CardTitle className="text-base">Generated QR fixtures</CardTitle><Badge variant="outline">SYNTHETIC</Badge></div><CardDescription>{QR_FIXTURES.length} QR labels use the same verification checks and open the same Control Tower workflow.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2">{QR_FIXTURES.map((fixture) => <div key={fixture.id} className="rounded-xl border p-3"><div className="mb-2 flex items-start justify-between gap-2"><div><p className="font-medium">{fixture.title}</p><p className="text-xs text-muted-foreground">{fixture.description}</p></div><Badge variant={fixture.kind === "verified" ? "outline" : fixture.kind === "unknown" ? "destructive" : "secondary"}>{fixture.kind.replaceAll("_", " ")}</Badge></div><div className="rounded-md border bg-white p-2"><img src={withBasePath(fixture.assetPath)} alt={`QR code ${fixture.code}`} className="mx-auto h-auto w-full max-w-[220px]" /></div><div className="mt-2 flex items-center justify-between gap-2 text-xs text-muted-foreground"><span>Lane: {fixture.expectedLane}</span><span>{fixture.packageCondition}</span></div><Button size="sm" className="mt-3 w-full" variant="outline" onClick={() => { setManualCode(fixture.code); setResult(null); setExceptionCreated(false); void verify(fixture.code, "fixture", fixture, "QR_CODE"); }}><ScanBarcode className="mr-2 h-4 w-4" />Test QR label</Button></div>)}</CardContent></Card></div>
      ...
      {result && <Card className={result.outcome === "VERIFIED" ? "border-emerald-500/40" : "border-amber-500/40"}><CardHeader><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2">{outcomeIcon(result.outcome)}<CardTitle className="text-base">Verification result</CardTitle><Badge variant={outcomeVariant(result.outcome)}>{formatOutcome(result.outcome)}</Badge><Badge variant="outline">{result.format === "QR_CODE" ? "QR_CODE" : "BARCODE"}</Badge><Badge variant="outline">{result.source}</Badge></div><span className="font-mono text-sm">{result.code}</span></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">Decision</p><p className="mt-1 font-medium">{result.message}</p></div><div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">Next action</p><p className="mt-1 font-medium">{result.nextAction}</p></div><div className="rounded-lg bg-muted/50 p-3"><p className="text-xs text-muted-foreground">OMS/WMS record</p><p className="mt-1 font-medium">{result.order ? `${result.order.fulfillmentStatus} · ${result.order.routePlanId ? `Route stop ${result.order.stopSequence ?? "—"}` : "Unassigned"}` : "Synthetic fixture / not found"}</p></div></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{result.checks.map((check) => <div key={check.key} className="rounded-lg border p-2"><p className="text-xs text-muted-foreground">{check.label}</p><p className={`mt-1 text-sm font-medium ${check.status === "PASS" ? "text-emerald-700" : check.status === "FAIL" ? "text-destructive" : "text-amber-700"}`}>{check.status}</p></div>)}</div>{result.fixture && <p className="text-sm text-muted-foreground">Expected dispatch lane: <span className="font-medium text-foreground">{result.fixture.expectedLane}</span> · Package condition: <span className="font-medium text-foreground">{result.fixture.packageCondition}</span></p>}{result.outcome !== "VERIFIED" && <div className="flex flex-wrap gap-2"><Button variant="destructive" onClick={() => void createException()} disabled={exceptionCreated}>{exceptionCreated ? <><CheckCircle2 className="mr-2 h-4 w-4" />Exception created</> : <><TriangleAlert className="mr-2 h-4 w-4" />Send to Control Tower</>}</Button><Button variant="outline" render={<Link href="/control-tower" />}><ExternalLink className="mr-2 h-4 w-4" />Open Control Tower</Button></div>}</CardContent></Card>}
      <Card><CardHeader><div className="flex items-center gap-2"><CardTitle className="text-base">Recent verification activity</CardTitle><Badge variant="outline">PERSISTED</Badge></div><CardDescription>Every scan is retained with its source, decision, and operator for a traceable warehouse handoff.</CardDescription></CardHeader><CardContent>{history.length === 0 ? <p className="text-sm text-muted-foreground">No scans yet. Test a barcode or QR fixture to create the first record.</p> : <div className="space-y-2">{history.slice(0, 8).map((scan) => <div key={scan.id} className="flex flex-col gap-2 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><span className="font-mono text-sm">{scan.code}</span><Badge variant="outline">{scan.format}</Badge><Badge variant={scan.outcome === "VERIFIED" ? "default" : scan.outcome === "EXCEPTION" ? "destructive" : "secondary"}>{scan.outcome}</Badge></div><p className="truncate text-xs text-muted-foreground">{scan.message}</p></div><span className="shrink-0 text-xs text-muted-foreground">{scan.actorName} · {new Date(scan.createdAt).toLocaleTimeString()}</span></div>)}</div>}</CardContent></Card>
    </div>
  );
}
