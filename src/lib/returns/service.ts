...
export async function scanReturn(id: string, input: { code: string; hubCode: string; stage: string; format?: string; actorUserId?: string; source?: string; sku?: string }) {
  ...
  if (outcome === "VERIFIED") { const { recordReturnCustodyEvent } = await import("@/lib/custody/service"); await recordReturnCustodyEvent({ returnCaseId: id, eventType: input.stage.toUpperCase().includes("PICK") ? "RETURN_PICKED_UP" : "RETURN_RECEIVED", externalEventId: `return-scan:${scan.id}`, hubCode: input.hubCode, actorUserId: input.actorUserId, dataSource: (input.source ?? row.dataSource) as "LIVE" | "SYNTHETIC" | "FALLBACK" | "MANUAL", payload: { scanId: scan.id, outcome, stage: input.stage } }); }
...
export async function inspectReturnCase(id: string, input: Parameters<typeof inspectReturn>[0] & { actorUserId?: string }) {
  ...
  { const { recordReturnCustodyEvent } = await import("@/lib/custody/service"); await recordReturnCustodyEvent({ returnCaseId: id, eventType: "INSPECTION_COMPLETED", externalEventId: `return-inspection:${inspection.id}`, hubCode: row.expectedHub, actorUserId: input.actorUserId, dataSource: row.dataSource as "LIVE" | "SYNTHETIC" | "FALLBACK" | "MANUAL", payload: { result: result.result, reasons: result.reasons } }); }
