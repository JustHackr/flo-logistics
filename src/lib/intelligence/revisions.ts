...
import { assessRouteConditions } from "./service";
...
export async function createRouteRevisionPreview(routePlanId: string, userId: string) {
...
export async function rejectRouteRevision(revisionId: string, userId: string) {
  const revision = await prisma.routeRevision.findUnique({ where: { id: revisionId }, select: { id: true, status: true } });
  if (!revision) throw new Error("Route revision not found");
