...
export async function getCustodyParcel(idOrExternal: string): Promise<CustodyParcelView> {
  ...
  return { id: row.id, externalParcelId: row.externalParcelId, orderId: row.orderId, externalOrderId: row.order?.externalOrderId ?? null, returnCaseId: row.returnCaseId, externalReturnId: row.returnCase?.externalReturnId ?? null, barcode: row.barcode, qrPayload: row.qrPayload, sku: row.sku, serialNumber: row.serialNumber, originHub: row.originHub, destinationHub: row.destinationHub, currentState: row.currentState, currentLocation: row.currentLocation, currentCustodian: mapActor(row.currentCustodian), confidenceScore: row.confidenceScore, riskLevel: row.riskLevel as CustodyParcelView["riskLevel"], dataSource: row.dataSource as CustodyParcelView["dataSource"], updatedAt: row.updatedAt.toISOString(), events, anomalies, evidence: row.evidence.map(mapEvidence), responsibilityWindow, investigations: row.investigations.map((investigation) => ({ id: investigation.id, status: investigation.status, finding: investigation.finding, confidence: investigation.confidence, assignedToUserId: investigation.assignedToUserId, notes: parse(investigation.notesJson, []), updatedAt: investigation.updatedAt.toISOString() })) };
}
